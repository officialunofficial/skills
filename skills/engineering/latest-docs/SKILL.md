...
A foreground, single-library check — reported in the turn itself, not handed to a background
agent like [`research`](../research/SKILL.md), and nothing is saved to a file.

1. Take the library, and its version if the request names one, from the request itself — this
   skill fires ad hoc on a named library, not by scanning the project's manifest or lockfile. No
   version given: check against the latest release.
