...
4. **Branch — is this a multi-session build?**
   ...
   - **No** → **`/implement`** right here, without leaving this context window. Prefer **`/plan-and-execute`** over a bare `/implement` when the task earns a written plan but not full PRD ceremony, or when your harness splits a fast, cheap model tier from its primary one — it plans on the primary model, then hands the written plan to the fast tier for `/implement`'s discipline.
